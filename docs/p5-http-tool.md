...
默认情况下，HttpRequestTool 禁止访问 localhost、私有 IP、云元数据接口。但在某些场景下：
- 开发时需要访问本地服务（`localhost:8080`）
- 测试环境在内网 IP 段
- 有明确的内部 API 需要访问

用户可以通过配置**地址白名单**明确授权，绕过 SSRF 防护。

### 11.2 配置方式

在 `~/.rrclaw/config.toml` 中添加：

```toml
[security]
autonomy = "supervised"
allowed_commands = ["ls", "cat", "git", "cargo"]

# HTTP 请求白名单（host 列表，命中后跳过 SSRF 检查）
http_allowed_hosts = [
    "localhost",           # 本地服务
    "127.0.0.1",          # IPv4 loopback
    "::1",                # IPv6 loopback
    "192.168.1.100",      # 特定内网 IP
...
- **精确匹配**：`localhost` 匹配 `localhost:8080`
- **后缀匹配**：`internal.company.com` 匹配 `api.internal.company.com`
- 白名单仅在 Supervised/Full 模式生效（ReadOnly 模式仍拒绝所有 HTTP）
...
用户配置白名单
        │
        ▼
SecurityPolicy::http_allowed_hosts = ["localhost", "192.168.1.100"]
        │
        ▼
HttpRequestTool::pre_validate()
        │
...
| `src/security/policy.rs` | SecurityPolicy 新增 `http_allowed_hosts` |
| `src/tools/http.rs` | pre_validate 增加白名单检查逻辑 |

### 11.6 实现要点

#### 11.6.1 src/config/schema.rs
...
pub struct SecurityConfig {
    ...
    /// HTTP 请求白名单（host 列表，绕过 SSRF 检查）
    #[serde(default)]
    pub http_allowed_hosts: Vec<String>,
}
```

#### 11.6.2 src/security/policy.rs
...
impl SecurityPolicy {
    ...
    pub fn is_http_host_allowed(&self, host: &str) -> bool {
        self.http_allowed_hosts.iter().any(|h| {
...
#### 11.6.3 src/tools/http.rs pre_validate 修改

```rust
fn pre_validate(&self, args: &serde_json::Value, policy: &SecurityPolicy) -> Option<String> {
    // 1. ReadOnly 模式拒绝（不变）

    // 2. URL 解析 + scheme 检查（不变）

    // 3. 白名单检查（新增）
    let host = /* 获取 host */;
    if policy.is_http_host_allowed(host) {
        debug!("http_request: host {} 在白名单中，跳过 SSRF 检查", host);
        return None;
    }
...
```rust
#[test]
fn pre_validate_whitelist_allows() {
    let mut policy = full_policy();
    policy.http_allowed_hosts = vec!["localhost".to_string()];

    let tool = HttpRequestTool;
    let args = serde_json::json!({"url": "http://localhost:8080/api"});
    let result = tool.pre_validate(&args, &policy);
    assert!(result.is_none(), "白名单中的 host 应放行");
}

#[test]
fn pre_validate_whitelist_blocks_private_ip() {
    let mut policy = full_policy();
    policy.http_allowed_hosts = vec!["192.168.1.100".to_string()];

    let tool = HttpRequestTool;
    // 192.168.1.200 不在白名单，应被 SSRF 检查拦截
    let args = serde_json::json!({"url": "http://192.168.1.200/api"});
    let result = tool.pre_validate(&args, &policy);
    assert!(result.is_some());
}
```
...
**核心挑战**：pre_validate 返回的是字符串错误信息，LLM 难以区分"不可恢复错误"和"可配置错误"。

**解决方案**：在 ToolResult 中增加标识字段，让 LLM 知道这个请求可以通过配置解决。

### 12.3 实现方案

#### 12.3.1 ToolResult 扩展
...
#### 12.3.2 HttpRequestTool pre_validate 返回值

```rust
fn pre_validate(&self, args: &serde_json::Value, policy: &SecurityPolicy) -> Option<String> {
    // ... 现有逻辑 ...

    // SSRF 检查拦截时，返回带配置建议的错误
    if let Some(reason) = check_ssrf_risk(host) {
        // 返回可配置的拒绝原因
        return Some(format!(
            "{}|可使用 /config set security.http_allowed_hosts 添加到白名单",
            reason
        ));
    }
...
在 Agent Loop 中，解析 tool result：
- 如果 error 包含 `|可使用`，说明可通过配置解决
- LLM 询问用户
- 用户同意后，LLM 调用 ConfigTool 添加白名单
- 重新执行 http_request
...
http_request 返回错误：
"禁止访问私有IP 192.168.1.100（SSRF防护）|可使用 /config set security.http_allowed_hosts 添加到白名单"
...
| 文件 | 改动 |
|------|------|
| `src/tools/traits.rs` | ToolResult 新增 `config_suggestion: Option<String>` |
| `src/tools/http.rs` | pre_validate 返回值包含配置建议 |
| `src/agent/loop_.rs` | 解析 config_suggestion，供 LLM 决策 |

### 12.6 提交策略

| # | message | 内容 |
|---|---------|------|
| 1 | `feat: add config_suggestion to ToolResult` | traits.rs |
| 2 | `feat: return config suggestion in http_request` | http.rs pre_validate |
| 3 | `feat: handle config suggestion in agent loop` | agent/loop_.rs |
