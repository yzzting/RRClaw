# P4-E: Memory Tools 实现计划

## 背景

当前 Memory 系统只在后台自动存取（Agent loop 结尾自动 store 对话摘要，开头自动 recall 相关记忆）。LLM 无法主动决定记住什么、回忆什么、遗忘什么。

新增 3 个工具让 LLM 主动管理记忆：`memory_store`、`memory_recall`、`memory_forget`。

---

## 一、架构设计

### 核心问题：Tool 如何访问 Memory？

当前 `Tool::execute()` 签名只接收 `args` 和 `&SecurityPolicy`，没有 Memory 引用。解决方案：**在 Tool 构造时注入 `Arc<dyn Memory>`**（与 SelfInfoTool 注入 Config 同模式）。

```
main.rs
  └── Arc<SqliteMemory> as Arc<dyn Memory>
        ├── Box::new(memory.clone()) → Agent.memory（Agent 自动存取用）
        ├── MemoryStoreTool::new(memory.clone())  ← 共享
        ├── MemoryRecallTool::new(memory.clone()) ← 共享
        └── MemoryForgetTool::new(memory.clone()) ← 共享
```
...
use memory::{MemoryStoreTool, MemoryRecallTool, MemoryForgetTool};  // 新增
...
```rust
// src/main.rs — run_agent() 中
// 原来:
//   let tools = rrclaw::tools::create_tools(config.clone(), data_dir.clone(), log_dir.clone(), config_path.clone(), skills.clone());
// 改为:
let tools = rrclaw::tools::create_tools(
    ...
    config_path.clone(),
    skills.clone(),
    memory.clone() as Arc<dyn rrclaw::memory::Memory>,  // 新增参数
);
```

注意：`memory` 是 `Arc<SqliteMemory>`，需要转为 `Arc<dyn Memory>`。因为 `Memory for Arc<SqliteMemory>` 已实现（见 `src/memory/sqlite.rs`），这个转换是合法的。

实际上 `Arc<SqliteMemory>` 可以直接 coerce 为 `Arc<dyn Memory>`，写法：`memory.clone() as Arc<dyn rrclaw::memory::Memory>`。

---

## 四、System Prompt 更新（可选）

在 `build_system_prompt()` 的决策原则段中，可以追加一条提示 LLM 使用记忆工具的原则：

```rust
// src/agent/loop_.rs — build_system_prompt() 的决策原则段末尾追加：
"6. 善用记忆: 当用户告知偏好或重要信息时，用 memory_store 保存；不确定之前是否讨论过时，用 memory_recall 检索",
```

这是**可选改动**，不做也可以——LLM 会从 tool description 自行判断何时使用。
...
| 文件 | 改动 | 复杂度 |
|------|------|--------|
| `src/tools/memory.rs` | **新增** — 3 个 Tool 实现 + truncate 辅助函数 | 中 |
| `src/tools/mod.rs` | 添加 `pub mod memory;` + 注册 3 个 tool + 签名新增 `memory` 参数 | 低 |
| `src/main.rs` | `create_tools()` 调用新增 `memory` 参数 | 低 |
| `src/agent/loop_.rs` | （可选）决策原则追加记忆提示 | 低 |

**不需要改动**：Memory trait、SqliteMemory、Provider、Security、CLI、Config。
...
| # | 提交 | 说明 |
|---|------|------|
| 1 | `feat: add memory tools (store/recall/forget)` | src/tools/memory.rs 新增 3 个 Tool |
| 2 | `feat: register memory tools and update create_tools signature` | mod.rs + main.rs |
| 3 | `test: add memory tools unit tests` | 全部测试 |
...
1. **`Arc<dyn Memory>` 转换**: `memory.clone() as Arc<dyn Memory>` 即可。`Memory for Arc<SqliteMemory>` 已在 `src/memory/sqlite.rs` 实现。

2. **`create_tools()` 签名变更**: 新增 `memory: Arc<dyn Memory>` 参数，`main.rs` 调用处需同步更新。Telegram channel 如果也调用 `create_tools` 也需更新。

3. **不需要改 Memory trait**: 3 个工具直接调用现有的 `store/recall/forget` 方法。

4. **tool name 不要与模块名冲突**: 文件名 `memory.rs` 在 tools 目录下，模块路径是 `crate::tools::memory`，不会与 `crate::memory` 冲突。

5. **MemoryCategory 默认值**: store 工具不传 category 时默认为 `Core`（核心知识），这是因为 LLM 主动存储的通常是重要信息。
